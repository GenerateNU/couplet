...
// 	assert.Equal(t, deletedEvent.ID, createdEvent.ID)
...
// 	assert.Error(t, err) // we WANT an error here
...
// 	assert.Error(t, err)
// 	assert.ErrorContains(t, err, "event with id="+badId.String()+" cannot be deleted because it doesn't exist")
